...
if WHICH_PYTHON == "":
    if OSX_APP_BUNDLED:
        proc = subprocess.Popen(
            ["export PATH=/usr/local/bin:$PATH;which python"],
            shell=True,
            universal_newlines=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        ...
        proc = subprocess.Popen(
            ["which python"], shell=True, universal_newlines=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
        )
        WHICH_PYTHON = proc.communicate()[0][:-1]
        ...
        if WHICH_PYTHON == "":
            INSTALLATION_ERROR_MESSAGE = (
                "Python 2.x doesn't seem to be installed "
...
if OSX_APP_BUNDLED:
    ...
    cmd = (
        "export -n PYTHONHOME PYTHONPATH EXECUTABLEPATH RESOURCEPATH ARGVZERO;"
...
if "ImportError" in IMPORT_PYO_STDERR:
    if "No module named" in IMPORT_PYO_STDERR:
        ...
        INSTALLATION_ERROR_MESSAGE = (
            "The current Python installation is running " "in 64-bit mode but pyo installation is 32-bit.\n\n"
        )
        if PLATFORM == "darwin":
            INSTALLATION_ERROR_MESSAGE += (
    ...
    if "ImportError" in IMPORT_WX_STDERR:
        if "No module named" in IMPORT_WX_STDERR:
            ...
            CALLER_NEED_TO_INVOKE_32_BIT = True
            INSTALLATION_ERROR_MESSAGE = (
                "The current Python installation is running in " "64-bit mode but WxPython installation is 32-bit.\n\n"
            )
            if PLATFORM == "darwin":
                INSTALLATION_ERROR_MESSAGE += (
                    "'VERSIONER_PYTHON_PREFER_32_BIT=yes' " "will be invoked before calling python " "executable.\n\n"
    ...
    for rep in SNIPPET_BUILTIN_CATEGORIES:
        if not os.path.isdir(os.path.join(SNIPPETS_PATH, rep)):
            ...
            if WIN_APP_BUNDLED:
                ...
                for file in files:
                    shutil.copy(
                        os.path.join(os.getcwd(), "Resources", "snippets", rep, file), os.path.join(SNIPPETS_PATH, rep),
                    )
            else:
                files = [f for f in os.listdir(os.path.join(os.getcwd(), "snippets", rep)) if f[0] != "."]
                for file in files:
                    shutil.copy(
                        os.path.join(os.getcwd(), "snippets", rep, file), os.path.join(SNIPPETS_PATH, rep),
                    )
...
if not os.path.isdir(STYLES_PATH):
    ...
    if WIN_APP_BUNDLED:
        ...
        for file in files:
            shutil.copy(
                os.path.join(os.getcwd(), "Resources", "styles", file), os.path.join(STYLES_PATH, file),
            )
    else:
        files = [f for f in os.listdir(os.path.join(os.getcwd(), "styles")) if f[0] != "."]
        for file in files:
            shutil.copy(
                os.path.join(os.getcwd(), "styles", file), os.path.join(STYLES_PATH, file),
            )
...
class ManualFrame(wx.Frame):
    ...
    def onRun(self, evt):
        ...
        th = DocRunningThread(
            ...
            TEMP_PATH,
            self.which_python,
            self.osx_app_bundled,
            self.caller_need_to_invoke_32_bit,
            self.set_32_bit_arch,
        )
        th.start()
...
class DocRunningThread(threading.Thread):
    def __init__(
        self, path, cwd, which_python, osx_app_bundled, caller_need_to_invoke_32_bit, set_32_bit_arch,
    ):
        ...
        if self.osx_app_bundled:
            ...
            if self.caller_need_to_invoke_32_bit:
                self.proc = subprocess.Popen(
                    ["%s%s%s %s" % (prelude, self.set_32_bit_arch, self.which_python, self.path)],
                    universal_newlines=True,
                    shell=True,
                    cwd=self.cwd,
                    stdout=subprocess.PIPE,
                ...
                self.proc = subprocess.Popen(
                    ["%s%s %s" % (prelude, self.which_python, self.path)],
                    cwd=self.cwd,
                    universal_newlines=True,
                    shell=True,
                    stdout=subprocess.PIPE,
            ...
            if self.caller_need_to_invoke_32_bit:
                self.proc = subprocess.Popen(
                    ["%s%s %s" % (self.set_32_bit_arch, self.which_python, self.path)],
                    universal_newlines=True,
                    shell=True,
                    cwd=self.cwd,
                    stdout=subprocess.PIPE,
    ...
    def run(self):
        if OSX_APP_BUNDLED:
            ...
            if CALLER_NEED_TO_INVOKE_32_BIT:
                self.proc = subprocess.Popen(
                    ['%s%s%s -u "%s"' % (prelude, SET_32_BIT_ARCH, WHICH_PYTHON, self.path)],
                    universal_newlines=True,
                    shell=True,
                    cwd=self.cwd,
                    stdout=subprocess.PIPE,
                ...
                self.proc = subprocess.Popen(
                    ['%s%s -u "%s"' % (prelude, WHICH_PYTHON, self.path)],
                    cwd=self.cwd,
                    universal_newlines=True,
                    shell=True,
                    stdout=subprocess.PIPE,
            ...
            if CALLER_NEED_TO_INVOKE_32_BIT:
                self.proc = subprocess.Popen(
                    ['%s%s -u "%s"' % (SET_32_BIT_ARCH, WHICH_PYTHON, self.path)],
                    universal_newlines=True,
                    shell=True,
                    cwd=self.cwd,
                    stdout=subprocess.PIPE,
...
class MainFrame(wx.Frame):
        ...
        for folder in EXAMPLE_FOLDERS:
            ...
            for ex in sorted(
                [
                    ...
                    if exp[0] != "." and not exp.endswith("pyc")
                ]
            ):
    ...
    def buildDoc(self):
        self.doc_frame = ManualFrame(
            osx_app_bundled=OSX_APP_BUNDLED,
...
class MainPanel(wx.Panel):
    def __init__(self, parent, size=(1200, 800), style=wx.SUNKEN_BORDER):
        ...
        self.right_splitter.SplitHorizontally(
            self.notebook, self.outputlog, (self.GetSize()[1] * 4 // 5) - self.GetSize()[1],
        )
