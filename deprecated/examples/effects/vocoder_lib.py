...
class MyVocoder(PyoObject):
    ...
    def __init__(self, in1, in2, base=50, spread=1.5, q=5, num=20, mul=1, add=0):
        ...
        in1_fader, in2_fader, base, spread, q, mul, add, lmax = convertArgsToLists(
            self._in1_fader, self._in2_fader, base, spread, q, mul, add
        )
        ...
        for i in range(lmax):
            ...
            self._excs.append(
                Biquadx(wrap(in2_fader, i), freq=self._freqs[-1], q=wrap(q, i), type=2, stages=2, mul=self._amps[-1],)
            )
    ...
    def ctrl(self, map_list=None, title=None, wxnoserver=False):
        ...
        self._map_list = [
            SLMap(20.0, 250.0, "lin", "base", self._base),
            SLMap(0.5, 2.0, "lin", "spread", self._spread),
            SLMap(1.0, 50.0, "log", "q", self._q),
            SLMapMul(self._mul),
        ]
