...
If you are not sure which version of Python the `pip` command is bound to, you can
...
For instructions on building from the source package, see `Compiling pyo from sources <compiling.html>`_.
...
Pyo also offers some GUI facilities to control or visualize audio processing.
...
nothing about these files and therefore cannot automatically remove them. To avoid conflicts between
...
Under Windows, all you have to do is run the uninstaller that comes with pyo to 
remove all its components. It is located in the Python root directory (e.g., C:\\Python27) 
...
To be "pip" compliant, I've had to make a choice and remove the pre-built E-Pyo application
...
E-Pyo is to run this command in a terminal or a command prompt:
