...
Here are some tips to help you configure the audio input/output on Windows. 
Some of these procedures are also valid for other systems.

How to choose the audio host api on Windows
-------------------------------------------

Choosing a good audio API on Windows can turn out to be a real headache.

This document presents a script that will inspect your system and tell you whether:
...
In the hope that this will help you have a good experience with Pyo under Windows!
...
.. code-block:: python
    ...
    This script will check whether Pyo can run in duplex mode (both audio input and output)
    and will test every host API to help the user make their audio device choice.
    """
    import sys, time
    from pyo import *
...
there are some things you can do to make it work. Open the **Sound** window by double-clicking on the volume
icon and choose *Playback Devices*. Here, select your device and click on the *Properties* button. In 
the *advanced* tab, make sure that the sampling rate is the same as the one used by Pyo (Pyo defaults to
...
the buffer size of Pyo's Server in order to avoid glitches in the audio streams.
...
- **pa_list_devices()**: Prints the list of audio devices. The first column is the index of the device.
...
methods take the index of the desired device and must be called before booting the Server. E.g.:
.. code-block:: python
