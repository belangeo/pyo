...
f = Seq(seq=[1, 1, 1, 0.5, 0.5], poly=4).play()
f.time = Sig(0.01)
f.seq = [0.5, 0.5, 0.5, 1]
...
# VoiceManager
env = CosTable([(0, 0), (100, 1), (500, 0.5), (8192, 0)])
met = Metro()
vm = VoiceManager(met)
...
# Mixer
a0 = Sine(freq=100, mul=0.2)
b0 = FM(carrier=200, ratio=[0.5013, 0.4998], index=6, mul=0.2)
...
# Selector
a1 = Sine(freq=[100, 101], mul=0.3)
b1 = Noise(mul=0.1)
c1 = SineLoop(freq=[101, 102], mul=0.5)
lf1 = Sine(freq=0.1, add=1)
...
sines2 = SineLoop(freq=rnds, feedback=0.05, mul=0.15).out()
