...
Chorus_process_ii(Chorus *self) {
    float lfo, pos, val, fpart, inval;
    int i, j, ipart;
    
    float *in = Stream_getData((Stream *)self->input_stream);
    float dpth = PyFloat_AS_DOUBLE(self->depth);
...
Chorus_process_ai(Chorus *self) {
    float lfo, pos, val, fpart, inval, dpth;
    int i, j, ipart;
    
    float *in = Stream_getData((Stream *)self->input_stream);
    float *depth = Stream_getData((Stream *)self->depth_stream);
    float feed = PyFloat_AS_DOUBLE(self->feedback);
...
Chorus_process_ia(Chorus *self) {
    float lfo, pos, val, fpart, inval, feed;
    int i, j, ipart;
...
Chorus_process_aa(Chorus *self) {
    float lfo, pos, val, fpart, inval, dpth, feed;
    int i, j, ipart;
...
static PyObject * Chorus_deleteStream(Chorus *self) { DELETE_STREAM };
...
"Chorus objects. Chorus signal by x samples.",           /* tp_doc */
