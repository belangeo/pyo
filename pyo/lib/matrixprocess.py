...
class MatrixRec(PyoObject):
    ...
    def __init__(self, input, matrix, fadetime=0, delay=0):
        ...
        in_fader, matrix, fadetime, delay, lmax = convertArgsToLists(
            self._in_fader, matrix, fadetime, delay
        )
        self._base_objs = [
...
class MatrixRecLoop(PyoObject):
    ...
    def __init__(self, input, matrix):
        ...
        self._base_objs = [
            MatrixRecLoop_base(wrap(in_fader, i), wrap(matrix, i)) for i in range(len(matrix))
        ]
        self._trig_objs = Dummy([TriggerDummy_base(obj) for obj in self._base_objs])
        self._init_play()
...
class MatrixPointer(PyoObject):
    ...
    def __init__(self, matrix, x, y, mul=1, add=0):
        ...
        self._base_objs = [
            MatrixPointer_base(wrap(matrix, i), wrap(x, i), wrap(y, i), wrap(mul, i), wrap(add, i))
            for i in range(lmax)
        ]
        self._init_play()
...
class MatrixMorph(PyoObject):
    ...
    def __init__(self, input, matrix, sources):
        ...
        self._base_objs = [
            MatrixMorph_base(wrap(in_fader, i), wrap(matrix, i), self._base_sources)
            for i in range(len(matrix))
        ]
        self._init_play()
