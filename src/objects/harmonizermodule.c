...
Harmonizer_transform_ii(Harmonizer *self) {
    ...
    float trans = PyFloat_AS_DOUBLE(self->transpo);
    float feed = PyFloat_AS_DOUBLE(self->feedback);

    hertz = 8.1757989156437 * powf(1.0594630943593, 60+trans);
...
Harmonizer_transform_ai(Harmonizer *self) {
    ...
    float *in = Stream_getData((Stream *)self->input_stream);
    float *trans = Stream_getData((Stream *)self->transpo_stream);
    float feed = PyFloat_AS_DOUBLE(self->feedback);
	    
	float oneOnWinsize = 1.0 / self->winsize;
	float oneOnSr = 1.0 / self->sr;
    for (i=0; i<self->bufsize; i++) {
		hertz = 8.1757989156437 * powf(1.0594630943593, 60+trans[i]);
		ratio = hertz * INV_BASE_HERTZ;
...
Harmonizer_init(Harmonizer *self, PyObject *args, PyObject *kwds)
{
    ...
    if (! PyArg_ParseTupleAndKeywords(args, kwds, "O|OOfOO", kwlist, &inputtmp, &transpotmp, &feedbacktmp, &self->winsize, &multmp, &addtmp))
        return -1; 
    ...
    self->buffer = (float *)realloc(self->buffer, (self->sr+1) * sizeof(float));
