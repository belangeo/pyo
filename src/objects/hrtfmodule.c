...
typedef struct {
    ...
    float hrtf_left_impulses[16][128];
    float hrtf_right_impulses[16][128];
    int modebuffer[2];
    MYFLT *buffer_streams;
} Binauraler;
...
Binauraler_new(PyTypeObject *type, PyObject *args, PyObject *kwds) {
    int i, j, k;
    PyObject *inputtmp, *input_streamtmp, *azitmp=NULL, *eletmp=NULL;
    Binauraler *self;
    ...
    static char *kwlist[] = {"input", "azi", "ele", NULL};

    if (! PyArg_ParseTupleAndKeywords(args, kwds, "O|OO", kwlist, &inputtmp, &azitmp, &eletmp))
        Py_RETURN_NONE;
...
{"Binauraldata", T_OBJECT_EX, offsetof(Binauraler, Binauraldata), 0, "HRIR dataset."},
